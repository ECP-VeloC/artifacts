...
public:
    ...
    virtual bool mem_protect(int id, void *ptr, size_t count, size_t base_size, unsigned int flags=0, release_routine r_routine=NULL ) = 0;
    virtual int prefetch_enqueue(const char *name, int version, int region_id = 0) = 0;
    virtual int prefetch_start() = 0;
    virtual size_t recover_size(char * ckpt_name, int version, int region_id) = 0;
    virtual bool next_prefetched(const char *name, int version, int region_id = 0) = 0;
    virtual void get_trf_metrics(char * c) = 0;
