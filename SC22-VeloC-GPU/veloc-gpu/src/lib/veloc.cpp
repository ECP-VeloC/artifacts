...
extern "C" int VELOC_Mem_protect(int id, void *ptr, size_t count, size_t base_size, unsigned int flags, release_routine r) {
    return CLIENT_CALL(veloc_client->mem_protect(id, ptr, count, base_size, flags, r));
}
...
extern "C" int VELOC_Restart_test(const char *name, int version) {
	...
	return -1;
    return CLIENT_CALL(veloc_client->restart_test(name, version));
}
...
extern "C" bool VELOC_Recover_mem() {
...
extern "C" size_t VELOC_Recover_ckpt_size(const char * ckpt_name, int version, int region_id) {
    return veloc_client->recover_size((char *)ckpt_name, version, region_id);
}

extern "C" int VELOC_Recover_size(int id) {
    return CLIENT_CALL(veloc_client->recover_size(id));
}
...
extern "C" bool VELOC_Next_prefetched(const char *name, int version, int region_id) {
    return veloc_client->next_prefetched(name, version, region_id);
}

extern "C" int VELOC_Prefetch_enqueue(const char *name, int version) {
    return veloc_client->prefetch_enqueue(name, version);
}

extern "C" int VELOC_Prefetch_start() {
    return veloc_client->prefetch_start();
}

extern "C" bool VELOC_Restart(const char *name, int version) {
...
extern "C" void VELOC_Get_trf_metrics(char* c) {
    return veloc_client->get_trf_metrics(c);
}
