...
int VELOC_Mem_protect(int id, void *ptr, size_t count, size_t base_size, unsigned int flags=0, release_routine r = NULL);
...
// return the size of the data saved under ckpt_name, version, region_id or -1 if id does not exist
//   IN id - the id of data structure saved under id
size_t VELOC_Recover_ckpt_size(const char * ckpt_name, int version, int region_id);
// FIXME: extern "C" was throwing the error: more than one instance of overloaded function "VELOC_Recover_size" has "C" linkage
...
bool VELOC_Restart(const char *name, int version);

// Enqueue checkpoints for prefetching
int VELOC_Prefetch_enqueue(const char *name, int version);

// Start prefetch operation for all enqueued checkpoints
int VELOC_Prefetch_start();

bool VELOC_Next_prefetched(const char *name, int version, int region_id);

void VELOC_Get_trf_metrics(char *);
