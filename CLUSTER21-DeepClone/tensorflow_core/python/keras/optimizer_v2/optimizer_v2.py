...
class OptimizerV2(trackable.Trackable):
  ...
  def apply_gradients(self, grads_and_vars, name=None, ckpt=None):
    ...
    self.ckpt = ckpt
    self.no_tensors = len(grads_and_vars)
  ...
  def _distributed_apply(self, distribution, grads_and_vars, name, apply_state):
    ...
    for i in range(len(var_list)):
        ids[var_list[i].name] = i
        self.ckpt.tensors.append(var_list[i])
    grads_and_vars = zip(reduced_grads, var_list)

    @tf.function(experimental_relax_shapes=True)
    def tmci_save(var):
      self.ckpt.save_tensors(str(self.no_tensors), [var], tid = ids[var.name])
    ...
    def apply_grad_to_update_var(var, grad):
      ...
      if var.constraint is not None:
        with ops.control_dependencies([update_op]):
          result = var.assign(var.constraint(var))
      else:
        result = update_op
      with ops.control_dependencies([result]):
        return tf.cond(self.ckpt.active, lambda: tmci_save(var), lambda: tf.constant(False))
